...
def visualize_3d_packing(ULD_L: int, ULD_W: int, ULD_H: int, x_vals: list[int] , y_vals:list[int], z_vals: list[int], lengths: list[int], widths: list[int], heights: list[int], fitted_flags: list[int], title: str):
    # Create a PyVista plotter object
    plotter = pv.Plotter(title=title, window_size=(800, 600))

    # Add a 3D container box
    container_points = np.array([
        [0, 0, 0], [ULD_L, 0, 0], [ULD_L, ULD_W, 0], [0, ULD_W, 0],  # Bottom face
        [0, 0, ULD_H], [ULD_L, 0, ULD_H], [ULD_L, ULD_W, ULD_H], [0, ULD_W, ULD_H]  # Top face
    ], dtype=np.float32)
